...
import styles from "./PayRulesSection.module.css";
...
const PAY_MODE_OPTIONS = [
    { value: "fixed_hours",  label: "Fixed hours (flat per day)" },
    { value: "actual_hours", label: "Actual hours (scheduled window)" },
...
/**
 * Shared training pay-rules edit section used by both the create and edit pages.
 *
            ...
            {rules.length > 0 && (
                <div className={styles.rulesList}>
                    {rules.map((rule, index) => {
                        ...
                        const showFixed  = rule.payMode === "fixed_hours";
                        ...
                        return (
                            <div key={index} className={`${styles.ruleCard} ${cardClass}`}>
                                ...
                                <div className={showFixed ? styles.ruleRowThree : styles.ruleRow}>
                                    ...
                                    <div className={styles.fieldRow}>
                                        <label className={styles.fieldLabel}>Pay Mode</label>
                                        <select
                                            className={styles.fieldInput}
                                            value={rule.payMode}
                                            onChange={(e) => updateRule(index, "payMode", e.target.value)}
                                            disabled={isActionPending}
                                        >
                                            {PAY_MODE_OPTIONS.map((o) => (
                                                <option key={o.value} value={o.value}>{o.label}</option>
                                            ))}
                                        </select>
                                    </div>
                                    ...
                                    {showFixed && (
                                        <div className={styles.fieldRow}>
                                            <label className={styles.fieldLabel}>
                                                Fixed Hours <span className={styles.fieldRequired}>*</span>
                                            </label>
                                            <input
                                                ...
                                                step="0.5"
                                                placeholder="e.g. 8"
                                                value={rule.fixedHours}
                                                onChange={(e) => updateRule(index, "fixedHours", e.target.value)}
                                                disabled={isActionPending}
                                            />
